...
exports.File = exports.FileLoader = undefined;
...
class FileLoader {
  ...
  constructor(baseDir) {
    // TODO: use baseDir
    this.baseDir = '/Users/philipandrew/projects/git/graphql-fs';
  }

  makeFullPath(partialPath) {
    return _path2.default.join(this.baseDir, partialPath);
  }

  openFile(filePath) {
    const fullPath = this.makeFullPath(filePath);
    return _fsPromise2.default.open(fullPath, 'r+').then(fd => new File(fd, _path2.default.basename(fullPath), fullPath)).catch(e => {
  ...
  openDir(dirPath) {
    ...
    return _fsPromise2.default.stat(fullPath).then(stats => {
...
class Directory {
  ...
  contents({ includeFiles = true, includeDirs = true }) {
    ...
    return _fsPromise2.default.readdir(this.path).then(res => {
      const promises = [];
      res.forEach(fileName => {
        const fullPath = `./${this.name}/${fileName}`;
        promises.push(_fsPromise2.default.stat(fullPath).then(res => {
...
class File {
  ...
  read() {
    // memoize this
    return _fsPromise2.default.readFile(this.fd);
  }

  stats() {
    // memoize this
    return _fsPromise2.default.fstat(this.fd)
